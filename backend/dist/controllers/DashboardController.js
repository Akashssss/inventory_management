...
const subMonths = (d, n) => new Date(d.getFullYear(), d.getMonth() - n, d.getDate());
const toDateStr = (d) => d.toISOString().split("T")[0];
const toWeekday = (d) => d.toLocaleDateString("en-IN", { weekday: "short" });
class DashboardController {
    ...
    async getAdminStats(req, res, next) {
        try {
            ...
            const [todayTxs, yesterdayTxs, thisMonthTxs, lastMonthTxs, allTimeTxs, recentTxs, dailySales7Days, topProductsRaw, lowStockProducts, allProducts, allUsers,] = await Promise.all([
                Transaction_1.Transaction.aggregate([{ $match: { tenantId, createdAt: { $gte: todayStart, $lte: todayEnd } } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
                Transaction_1.Transaction.aggregate([{ $match: { tenantId, createdAt: { $gte: yesterdayStart, $lte: yesterdayEnd } } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
                Transaction_1.Transaction.aggregate([{ $match: { tenantId, createdAt: { $gte: thisMonthStart, $lte: now } } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
                Transaction_1.Transaction.aggregate([{ $match: { tenantId, createdAt: { $gte: lastMonthStart, $lte: lastMonthEnd } } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
                Transaction_1.Transaction.aggregate([{ $match: { tenantId } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
                Transaction_1.Transaction.find({ tenantId }).sort({ createdAt: -1 }).limit(10).lean().exec(),
                ...
                Product_1.Product.find({ tenantId, status: "active", $expr: { $lte: ["$stock", "$lowStockThreshold"] } })
                    .select("name stock lowStockThreshold images sellingPrice").sort({ stock: 1 }).limit(20).lean().exec(),
                Product_1.Product.countDocuments({ tenantId, status: "active" }),
            ...
            const chartData = Array.from({ length: 7 }, (_, i) => {
                ...
                const found = dailySales7Days.find((d) => d._id === dateStr);
    ...
    async getSellerStats(req, res, next) {
        try {
            ...
            const [todayTxs, allTimeSeller, dailySales7Days, topSoldItems, lowStockProducts, pendingReconciliation, recentTxs] = await Promise.all([
                Transaction_1.Transaction.aggregate([{ $match: { ...matchSeller, createdAt: { $gte: todayStart, $lte: todayEnd } } }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
                Transaction_1.Transaction.aggregate([{ $match: matchSeller }, { $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } } }]),
                Transaction_1.Transaction.aggregate([
                ...
                Transaction_1.Transaction.find(matchSeller).sort({ createdAt: -1 }).limit(5).lean().exec(),
            ]);
            const chartData = Array.from({ length: 7 }, (_, i) => {
                const date = subDays(now, 6 - i);
                const dateStr = toDateStr(date);
