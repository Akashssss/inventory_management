...
import { QueryOptions ,   SortSpec,
  ...
  FieldMetadata,
  IQueryValidator,
  FilterOperator, } from "../../types/query.types";
...
export class QueryValidator implements IQueryValidator {
  ...
  private maxFilterDepth: number = 10;
  private maxConditionsPerNode: number = 20;

  constructor(maxFilterDepth: number = 10, maxConditionsPerNode: number = 20) {
  ...
  private validateFilterNode(
    ...
    if (depth > this.maxFilterDepth) {
      throw new Error(
        `Filter nesting depth (${depth}) exceeds maximum (${this.maxFilterDepth})`
      );
    }
    ...
    if (node.logic && node.conditions) {
      ...
      if (node.conditions.length > this.maxConditionsPerNode) {
        throw new Error(
          `Too many conditions in group (${node.conditions.length}, max: ${this.maxConditionsPerNode})`
        );
      }
  ...
  private validateLeafCondition(
    ...
    if (!this.registry.supports(operator as FilterOperator)) {
      throw new Error(
        `Operator "${operator}" is not supported. Available: ${this.registry.list().join(", ")}`
      );
    }

    // Check operator is supported for this field type
    const supportedOps = fieldMeta.supportedOperators || this.registry.list();
    if (!supportedOps.includes(operator as FilterOperator)) {
      throw new Error(
        `Operator "${operator}" is not supported for field "${field}" (type: ${fieldMeta.type}). Supported: ${supportedOps.join(", ")}`
      );
    }

    // Type-specific validation
    this.validateOperatorValue(operator as FilterOperator, fieldMeta, value);
  }
  ...
  private validateOperatorValue(
    ...
    if (value === undefined || value === null) {
      throw new Error(
        `Operator "${operator}" requires a value, got ${value}`
      );
    }
  ...
  private validateNumberOperatorValue(
    ...
    if (["lt", "lte", "gt", "gte", "eq", "ne"].includes(operator)) {
      if (typeof value !== "number") {
        throw new Error(
          `Operator "${operator}" requires a number, got ${typeof value}`
        );
      }
      ...
      if (
        ...
        throw new Error(
          `Operator "between" requires [min: number, max: number]`
        );
      }
    } else if (["in", "notIn"].includes(operator)) {
      if (!Array.isArray(value) || !value.every((v) => typeof v === "number")) {
        throw new Error(
          `Operator "${operator}" requires an array of numbers`
        );
      }
    }
  }
  ...
  private validateDateOperatorValue(
    ...
    if (["dateIsBefore", "dateIsAfter", "eq", "ne"].includes(operator)) {
      ...
      if (
        ...
        throw new Error(
          `Operator "dateIsBetween" requires [startDate, endDate]`
        );
      }
    } else if (operator === "isRelativeToToday") {
      if (typeof value !== "object" || !value.operator) {
        throw new Error(
          `Operator "isRelativeToToday" requires { operator: string, days?: number }`
        );
      }
    }
  }
  ...
  private validateStringOperatorValue(
    ...
    if (
      ...
      if (typeof value !== "string" && typeof value !== "object") {
        throw new Error(
          `Operator "${operator}" requires a string or regex pattern`
        );
      }
    } else if (["in", "notIn"].includes(operator)) {
  ...
  private validateBooleanOperatorValue(
    ...
    if (["eq", "ne"].includes(operator)) {
      if (typeof value !== "boolean") {
        throw new Error(
          `Operator "${operator}" requires a boolean value for boolean field`
        );
      }
    }
  }
  ...
  validateSort(sort: SortSpec[] | undefined, schema: FieldMetadata[]): void {
    ...
    for (const spec of sort) {
      ...
      if (!["asc", "desc"].includes(spec.order)) {
        throw new Error(
          `Invalid sort order "${spec.order}". Use "asc" or "desc"`
        );
      }
    }
  }
   ...
   * @private
   */
  private estimateFilterComplexity(node: FilterNode, depth: number = 1): number {
    let score = 0;
    ...
    if (node.logic && node.conditions) {
      ...
      if (["regex", "contains", "notContains"].includes(node.operator)) {
        ...
        ["startsWith", "endsWith", "isRelativeToToday"].includes(node.operator)
      ) {
        score += 2;
      } else {
        score += 1;
