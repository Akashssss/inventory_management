...
import { toAppException } from "@/exceptions/AppException";
...
export class ProductController {
   ...
   * Create new product
   */
  async getSmallProductCategories(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
  ...
  async getSmallProductPrices(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
  ...
  async createProduct(
    ...
    try {
      ...
      if (productData.itemsPerBox) productData.itemsPerBox = Number(productData.itemsPerBox);
      if (productData.boxCostPrice) productData.boxCostPrice = Number(productData.boxCostPrice);

      // Handle boolean strings from FormData
      if (typeof productData.comesInBoxes === 'string') productData.comesInBoxes = productData.comesInBoxes === 'true';
      if (typeof productData.isSmallProduct === 'string') productData.isSmallProduct = productData.isSmallProduct === 'true';
      ...
      if (productData.type === "non-measurable") {
        ...
        const setting = await Setting.findOne({ tenantId }).exec();
  ...
  async updateProduct(
    ...
    try {
      ...
      if (updateData.itemsPerBox !== undefined) updateData.itemsPerBox = Number(updateData.itemsPerBox);
      if (updateData.boxCostPrice !== undefined) updateData.boxCostPrice = Number(updateData.boxCostPrice);
      if (updateData.lowStockThreshold !== undefined) updateData.lowStockThreshold = Number(updateData.lowStockThreshold);

      // Handle boolean strings from FormData
      if (typeof updateData.comesInBoxes === 'string') updateData.comesInBoxes = updateData.comesInBoxes === 'true';
      if (typeof updateData.isSmallProduct === 'string') updateData.isSmallProduct = updateData.isSmallProduct === 'true';
  ...
  async searchProducts(
    ...
    try {
      ...
      const query = (q || search || body.q || body.search || "") as string;
      ...
      if (options.filter && (options.filter as any).conditions) {
        (options.filter as any).conditions = (options.filter as any).conditions.map((cond: any) => {
  ...
  async getLowStock(
    ...
    try {
      ...
      const result = await this.repository.findLowStock(
  ...
  async advancedSearch(
    ...
    try {
      ...
      const result = await this.repository.findByAdvancedCriteria(
        category,
        minPrice,
        maxPrice,
        minStock,
